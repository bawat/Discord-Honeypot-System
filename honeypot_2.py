...
def HONEYPOT_2():
    print("[HONEYPOT_2] Establishing connection to bot_instance...")
    time.sleep(0.2)
    print("[HONEYPOT_2] Connection established to bot_instance.")
    time.sleep(0.5)


def load_token_config(file_name):
    with open(os.path.join('Tokens', file_name), 'r') as f:
        print(f"[HONEYPOT_2] Config {file_name} was loaded.")
        return json.load(f)
...
token_data = load_token_config('token_2.json')
...
async def change_profile():
    while True:
        ...
        try:
            ...
            print(f"[HONEYPOT_2] Changed username to {new_name}")
        except discord.HTTPException as e:
            print(f"[HONEYPOT_2] Failed to change username: {e}")

        try:
            with open(new_image_path, 'rb') as img:
                await bot.user.edit(avatar=img.read(), password=token_data['password'])
                print(f"[HONEYPOT_2] Changed avatar image to {new_image_path}")
        except discord.HTTPException as e:
            print(f"[HONEYPOT_2] Failed to change avatar: {e}")

        # Update the bio using the external function
        status_code = update_bio(bot_token, new_bio)
        if status_code != 200:
            print(f"[HONEYPOT_2] Failed to update bio: HTTP {status_code}")
...
async def change_status():
    while True:
        ...
        print(f"[HONEYPOT_2] Changed status to {new_status}")
...
@bot.event
async def on_ready():
    print(f'[HONEYPOT_2] Logged in as {bot.user.name}. Changing username...')
    bot.loop.create_task(change_profile())
    print(f'[HONEYPOT_2] Attempting to change avatar and username')
    bot.loop.create_task(change_status())
    print(f'[HONEYPOT_2] Attempting to change status')
...
async def on_message(message):
    if not message.guild:  # Check if the message is a DM
        ...
        log_entry_str = f"[HONEYPOT_2] DM Received - Time: {message.created_at}, From: {message.author} (ID: {message.author.id}, Avatar URL: {avatar_url}, Message: {message.content}"
        ...
        logs_path = os.path.join('Honeypot-Logs', "honeypot_dm_logs_2.json")
        ...
        if guild_id:
            ...
            if guild:
                try:
                    ...
                    print(f"[HONEYPOT_2] Kicked {message.author} from guild.")
                    await send_kill_message(member)
                except discord.NotFound:
                    print(f"[HONEYPOT_2] {message.author} is not in the guild, skipping kick.")
                except discord.Forbidden:
                    print(f"[HONEYPOT_2] Missing permissions to kick {message.author}.")
                except discord.HTTPException as e:
                    print(f"[HONEYPOT_2] Failed to kick {message.author}: {e}")
...
async def send_kill_message(killed_member):
    ...
    content = (
        f"```ansi\n"
        f"\033[1;37m{bot_nick}\033[0m killed \033[1;37m{killed_member.name}\033[0m \033[1;33m(Battle Axe)\033[0m\n"
        f"```"
    )
    for channel_id in [1174037408128446556, 1514292468210733217]:
        ...
        if channel:
            try:
                ...
                print(f"[HONEYPOT_2] Failed to send kill message to {channel_id}: {e}")


async def send_dm_log_to_channel(log_entry):
    channel_id = token_data.get('channel_id')
    if not channel_id:
        print("[HONEYPOT_2] Channel ID is not set in token.json.")
        return

    channel = bot.get_channel(int(channel_id))
    if not channel:
        print(f"[HONEYPOT_2] Channel with ID {channel_id} not found.")
        return

    message_content = (
        "## HONEYPOT NO.2 WAS TRIGGERED\n"
        f"- **HONEYPOT NO.2 RUNNING AS: @{bot.user.name}**\n"
...
while True:
    try:
        ...
        print("[HONEYPOT_2] Token expired, refreshing...")
        new_token = refresh_token('token_2.json')
        if new_token is None:
            print("[HONEYPOT_2] Token refresh failed, exiting.")
            break
        bot_token = new_token
        token_data['token'] = new_token
        print("[HONEYPOT_2] Token refreshed, reconnecting...")
