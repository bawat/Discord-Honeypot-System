        ...
        if guild_id:
            ...
            if guild:
                try:
                    ...
                    print(f"[HONEYPOT_5] Kicked {message.author} from guild.")
                    await send_kill_message(member)
                except discord.NotFound:
                    print(f"[HONEYPOT_5] {message.author} is not in the guild, skipping kick.")
                except discord.Forbidden:
                    print(f"[HONEYPOT_5] Missing permissions to kick {message.author}.")
                except discord.HTTPException as e:
                    print(f"[HONEYPOT_5] Failed to kick {message.author}: {e}")
...
async def send_kill_message(killed_member):
    ...
    content = (
        f"```ansi\n"
        f"\033[1;37m{bot_nick}\033[0m killed \033[1;37m{killed_member.name}\033[0m \033[1;33m(Arming Sword)\033[0m\n"
        f"```"
    )
    for channel_id in [1174037408128446556, 1514292468210733217]:
        ...
        if channel:
            try:
                ...
                print(f"[HONEYPOT_5] Failed to send kill message to {channel_id}: {e}")
...
async def send_dm_log_to_channel(log_entry):
    ...
    message_content = (
        "## HONEYPOT NO.5 WAS TRIGGERED\n"
        f"- **HONEYPOT NO.5 RUNNING AS: @{bot.user.name}**\n"
...
while True:
    try:
        ...
        print("[HONEYPOT_5] Token expired, refreshing...")
        new_token = refresh_token('token_5.json')
        if new_token is None:
            print("[HONEYPOT_5] Token refresh failed, exiting.")
            break
        bot_token = new_token
        token_data['token'] = new_token
        print("[HONEYPOT_5] Token refreshed, reconnecting...")
